from dagster import asset, AssetExecutionContext, Definitions, define_asset_job, AssetSelection, ScheduleDefinition
import requests
...
def get_urls(context: AssetExecutionContext, get_paths: list):
    ...
    for path in get_paths:
        ...
        soup_for_page = BeautifulSoup(get_page_content(base_url.format(item_path = path, page = 1)), 'lxml')

        pagination = soup_for_page.find_all('div', class_='pagination_pagination__wJ_sG')

        last_page = re.findall('page=[0-9]*\"', str(pagination[0]))[-1].split("=")[-1][:-1]
...
def process_bigc_data(context: AssetExecutionContext, get_page_contents: list):
    ...
    for page_content in get_page_contents:
        ...
        for product in products:
            try:
                name_tag = product.find('div', class_='productCard_title__f1ohZ').text.strip()
                price_tag = product.find('div', class_='productCard_price__9T3J8').text.strip()
...
def upload_file_drive(context: AssetExecutionContext, process_bigc_data : str):
    ...
    media = MediaFileUpload('./data/'+process_bigc_data, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    file = service.files().create(body=file_metadata, media_body=media, fields='id').execute()
    
    
defs = Definitions(
    assets=[get_paths, get_urls, get_page_contents, process_bigc_data, upload_file_drive],
    jobs = [
        define_asset_job(
            name="process_bigc_data_and_upload",
